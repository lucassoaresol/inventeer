---
name: apex-eng-ready
description: "APEX · Per-task readiness gate — one cheap ✅/⚠️/❌ sweep (setup, context, connections, baseline, task) that tells you if you're ready to start, and routes to the missing step if not. Reusable standalone and as eng-start Phase 0. Wrapper experimental: inspeciona apex://framework/workflows/eng-ready no Codex; não use como executor de entrega."
---
...
1. Use este wrapper somente quando o usuário pedir inspeção ou diagnóstico explícito da integração
   APEX no Codex.
2. Leia o recurso MCP `apex://framework/workflows/eng-ready` e pare se o servidor, recurso ou conteúdo não estiver
   disponível.
