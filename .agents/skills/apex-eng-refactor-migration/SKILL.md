...
description: "APEX · Plan and execute large-scale refactoring or migration safely — characterization tests, seam identification, strangler-fig incremental cutover, and rollback gating. Wrapper experimental: inspeciona apex://framework/workflows/eng-refactor-migration no Codex; não use como executor de entrega."
---
...
1. Use este wrapper somente quando o usuário pedir inspeção ou diagnóstico explícito da integração
   APEX no Codex.
2. Leia o recurso MCP `apex://framework/workflows/eng-refactor-migration` e pare se o servidor, recurso ou conteúdo não estiver
   disponível.
