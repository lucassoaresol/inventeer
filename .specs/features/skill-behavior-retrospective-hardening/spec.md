...
- [x] Make TLC completion fail closed when Verifier independence is undeclared or unsupported.
- [x] Merge future lesson recurrences by a stable semantic pattern across distinct features.
- [x] Close the PR-review pilot with exact-head materialization and explicit local-validation reasons.
- [x] Provide a discoverable, sanitized retrospective skill backed by conservative engine metrics.
